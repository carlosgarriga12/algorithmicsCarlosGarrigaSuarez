...
public abstract class Vector implements ISortingProblem {
	...
	*/
	...
	/**
	 * It returns the position of the smallest value in
	 * the list starting from firstPos
	 * @param firstPos
	 */
	protected int findPosMin(int firstPos) {
		int data = Integer.MAX_VALUE;
		int position = Integer.MAX_VALUE;
		
		for(int i = firstPos; i < elements.length; i++) {
			if (elements[i] < data) {
				position = i;
				data = elements[i];
			}
